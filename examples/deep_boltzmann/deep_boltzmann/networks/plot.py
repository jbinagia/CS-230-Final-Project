...
def test_xz_projection(Txz, xtrajs, rctrajs=None, subplots=None, colors=None, density=False):
    ...
    if subplots[3]:
        ...
        for ztraj, color in zip(ztrajs, colors):  # plt.rcParams['axes.prop_cycle'].by_key()['color']
            axes[cplot].hist(ztraj.dot(lr.coef_) + lr.intercept_, 50, linewidth=2, alpha=0.2, color=color)
            axes[cplot].hist(ztraj.dot(lr.coef_) + lr.intercept_, 50, histtype='step', linewidth=2, color=color)
